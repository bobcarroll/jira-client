...
public class RestClient {
  ...
  private JSON request(HttpRequestBase req) throws RestException, IOException {
    ...
    if (ent != null) {
      ...
      InputStreamReader isr =
          encoding != null ? new InputStreamReader(ent.getContent(), encoding)
    ...
    if (sl.getStatusCode() >= 300)
      throw new RestException(sl.getReasonPhrase(), sl.getStatusCode(),
                              result.toString(), resp.getAllHeaders());

    return result.length() > 0 ? JSONSerializer.toJSON(result.toString())
                               : null;
  }
  ...
  private JSON request(HttpEntityEnclosingRequestBase req,
    ...
    if (attachments != null) {
      ...
      for (Issue.NewAttachment attachment : attachments) {
        ...
        if (content instanceof byte[]) {
          ...
          ent.addPart("file",
                      new InputStreamBody((InputStream)content, filename));
        } else if (content instanceof File) {
          ent.addPart("file", new FileBody((File)content, filename));
        } else if (content == null) {
          throw new IllegalArgumentException("Missing content for the file " +
                                             filename);
        } else {
   ...
   *     URI
   */
  public JSON delete(String path)
   ...
   */
  public JSON post(String path, File file)
      throws RestException, IOException, URISyntaxException {
